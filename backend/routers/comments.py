...
def create_comment(post_id: int, comment_data: schemas.CommentCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    # Format manually to avoid missing fields like user and like_count
    return {
        "id": new_comment.id,
...
def get_comments(post_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_optional_user)):
    """Lấy toàn bộ comments của post (bao gồm replies) với 1 query duy nhất và dựng cây tree trong RAM"""
    ...
    ).options(joinedload(models.Comment.user)).filter(models.Comment.post_id == post_id)
    ...
    # Dựng cấu trúc cây trong RAM (O(N))
    comment_dict = {}
    for row in results:
        if current_user:
...
def like_comment(comment_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    if existing:
        # Nếu đã like rồi thì click lại sẽ là unlike
        db.delete(existing)
        db.commit()
